				...
				<div class="col-md-6 col-sm-6 col-xs-12">
					<div class="panel panel-info">
						...
						<div class="panel-body">
							<form role="form" action="update-customer-info-process.jsp" method="">
								<%							
								String cid = request.getParameter("cid");
								ResultSet resultset = DatabaseConnection.getResultFromSqlQuery("select * from customer where c_id='" + cid + "'");
								while (resultset.next()) {
								%>
								<div class="form-group">
									<label>Customer Id</label> <input class="form-control" type="text" name="cid" value="<%=resultset.getString(1) %>" readonly/>
								</div>
								<div class="form-group">
									<label>Customer Name</label> <input class="form-control" type="text" name="cname" value="<%=resultset.getString(2) %>" />
								</div>
								<div class="form-group">
